...
const ArticleList: FC = () => {
  ...
  const visibleTags = useMemo(() => {
    if (areTagsExpanded) return tags;

    const collapsedTags = tags.slice(0, 14);
    if (activeTag && !collapsedTags.includes(activeTag)) {
      return [...collapsedTags, activeTag];
    }
    return collapsedTags;
  }, [tags, areTagsExpanded, activeTag]);
  ...
  }, [articles, activeCategory, activeTag, searchTerm, sortBy]);
  ...
  const showLoadMore = hasMoreArticles && activeCategory === "全部" && activeTag === "" && searchTerm.trim() === "";
  ...
  return (
    <div className={`articles-container ${isAnimated ? 'animated' : ''}`}>
      <div className="articles-header">
        ...
        <div className="article-stats">
          <div className="stat-item">
            ...
            <span className="stat-label">{articleTotal > articles.length ? `已加载 / 共 ${articleTotal}` : '文章总数'}</span>
          </div>
          <div className="stat-item">
      ...
      <div className="search-filter-container">
        ...
        {tags.length > 0 && (
          <div className={`tag-filter-panel ${areTagsExpanded ? 'expanded' : 'collapsed'}`}>
            ...
            {tags.length > 14 && (
              ...
              >
                {areTagsExpanded ? <FaChevronUp /> : <FaChevronDown />}
                <span>{areTagsExpanded ? '收起标签' : `展开全部标签（${tags.length}）`}</span>
              </button>
            )}
          </div>
        )}
      ...
      {showLoadMore && (
        <div className="load-more-container">
          ...
          >
            {isLoadingMore && <FaSpinner className="load-more-spinner" />}
            <span>{isLoadingMore ? '加载中...' : `加载更多（${articles.length}/${articleTotal || articles.length}）`}</span>
          </button>
        </div>
      )}
      <ScrollToTopButton />
